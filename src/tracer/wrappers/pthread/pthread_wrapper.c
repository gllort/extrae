...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/trunk/src/tracer/wrappers/pthread/pthread_wrapper.c $
 | @last_commit: $Date: 2016-01-13 16:16:18 +0100 (mié, 13 ene 2016) $
 | @version:     $Revision: 3761 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: pthread_wrapper.c 3761 2016-01-13 15:16:18Z harald $";
