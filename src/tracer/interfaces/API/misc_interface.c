...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 ...
 | @last_commit: $Date: 2015-11-19 18:19:59 +0100 (jue, 19 nov 2015) $
 | @version:     $Revision: 3652 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: misc_interface.c 3652 2015-11-19 17:19:59Z harald $";
