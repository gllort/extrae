...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 ...
 | @last_commit: $Date: 2015-11-10 16:10:00 +0100 (mar, 10 nov 2015) $
 | @version:     $Revision: 3568 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#include "common.h"

static char UNUSED rcsid[] = "$Id: paraver_state.c 3568 2015-11-10 15:10:00Z harald $";
